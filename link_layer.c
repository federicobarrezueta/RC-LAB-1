...
static int receiveSUFrame(unsigned char expectedC) {
    ...
    while (state != END) {
        ...
        switch (state) {
            ...
            case FLAG_RCV:
                if      (buf == FLAG)  state = FLAG_RCV;
                else if (buf == A_TX || buf == A_RX) { a_rcv = buf; state = A_RCV; }
                else                   state = START;
                break;
            case A_RCV:
                if      (buf == expectedC) { c_rcv = buf; state = C_RCV; }
...
int llopen(LinkLayer params) {
    ...
    if (fd < 0) { perror(ll.serialPort); return -1; }
    ...
    if (ll.role == TRANSMITTER) {
        ...
        while (alarmCount <= ll.numTransmissions) {
            ...
            switch (state) {
                ...
                case FLAG_RCV:
                    if      (buf == FLAG)  state = FLAG_RCV;
                    else if (buf == A_RX)  { a_rcv = buf; state = A_RCV; }
                    else                   state = START;
                    break;
                case A_RCV:
                    if      (buf == C_UA)  { c_rcv = buf; state = C_RCV; }
                    else if (buf == FLAG)  state = FLAG_RCV;
                    else                  state = START;
                    break;
                case C_RCV:
...
int llwrite(const unsigned char *buf, int bufSize) {
    ...
    while (!ackReceived && alarmCount <= ll.numTransmissions) {
        ...
        switch (state) {
            ...
            case FLAG_RCV:
                if      (byte == FLAG)  state = FLAG_RCV;
                else if (byte == A_TX)  { a_rcv = byte; state = A_RCV; }
                else                    state = START;
                break;
            case A_RCV:
                if (byte == expectedRR || byte == expectedREJ) {
                    c_rcv = byte; state = C_RCV;
                } else if (byte == FLAG) state = FLAG_RCV;
                else                     state = START;
                break;
            case C_RCV:
                if      (byte == (a_rcv ^ c_rcv)) state = BCC_OK;
                else if (byte == FLAG)             state = FLAG_RCV;
                else                               state = START;
                break;
            case BCC_OK:
    ...
    alarm(0);
    if (!ackReceived) { printf("llwrite: failed\n"); return -1; }
    Ns = !Ns;
    return frameLen;
}
...
int llread(unsigned char *packet) {
    ...
    while (1) {
        ...
        switch (state) {
            ...
            case FLAG_RCV:
                if      (buf == FLAG)  state = FLAG_RCV;
                else if (buf == A_TX)  { a_rcv = buf; state = A_RCV; }
                else                   state = START;
                break;
            case A_RCV:
                ...
                break;
            case C_RCV:
                if      (buf == bcc1_rcv) { state = BCC_OK; dataIdx = 0; escaped = FALSE; }
                else if (buf == FLAG)      state = FLAG_RCV;
                else                       state = START;
                break;
            case BCC_OK:
